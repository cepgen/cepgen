...
namespace cepgen::pythia6 {
  ...
  public:
    ...
    inline static ParametersDescription description() {
      ...
      desc.add("nf", mstu(112)).setDescription("nominal number of ﬂavours assumed in alpha(s) expression");
      desc.add("minNf", mstu(113))
          .setDescription("minimum number of ﬂavours that may be assumed in alpha(S) expression");
      desc.add("maxNf", mstu(114))
          .setDescription("minimum number of ﬂavours that may be assumed in alpha(S) expression");
      desc.add("singularityTreatment", mstu(115))
      ...
      desc.add("fixedAlphaS", paru(111))
          .setDescription(
              "fix alpha(S) value assumed when order=0 (and also in parton showers when alpha(S) is assumed fix "
              "there)");
      desc.add("Lambda", paru(112)).setDescription("Lambda value used in running");
      desc.add("flavourThreshold", paru(113))
          .setDescription(
              "flavour threshold, for the effective number of flavours 'nf' to use (='flavourThreshold'*m_q^2)");
      desc.add("minQ2", paru(114)).setDescription("Q^2 value below which alpha(S) is assumed constant");
      desc.add("maxQ2", paru(115)).setDescription("maximum alpha(S) value computable");
      return desc;
    }
