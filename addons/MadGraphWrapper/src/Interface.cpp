...
void Interface::generateLibrary(const fs::path& proc_path, const fs::path& in_path, const fs::path& out_lib) const {
...
ParametersDescription Interface::extractParamCardParameters(const std::string& card_content) {
  ...
  for (const auto& buf : utils::split(card_content, '\n', true)) {
    ...
    if (vars.size() == 2) {
      ...
      throw CG_ERROR("mg5amc:Interface:extractParamCardParameters") << "Failed to unpack parameters line:\n\t" << buf;
  }
  if (!block_params.empty())
    output.add(block_name, block_params);
