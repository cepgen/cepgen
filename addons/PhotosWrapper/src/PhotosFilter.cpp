...
namespace cepgen::photos {
  ...
  class PhotosFilter final : public EventModifier {
  public:
    explicit PhotosFilter(const ParametersList& params) : EventModifier(params) {
      if (steer<bool>("debug"))
        Log::LogAll(true);
    ...
    ~PhotosFilter() override { Log::SummaryAtExit(); }
