    ...
    double PPtoWW::offShellME(double phi_sum, double phi_diff) const {
      ...
      for (const auto lam3 : pol_w1_)
        for (const auto lam4 : pol_w2_) {
          double ampli_pp = ampl_(s_hat, t_hat, u_hat, +1, +1, lam3, lam4);
          double ampli_mm = ampl_(s_hat, t_hat, u_hat, -1, -1, lam3, lam4);
          double ampli_pm = ampl_(s_hat, t_hat, u_hat, +1, -1, lam3, lam4);
          double ampli_mp = ampl_(s_hat, t_hat, u_hat, -1, +1, lam3, lam4);
