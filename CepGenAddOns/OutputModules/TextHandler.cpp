...
namespace cepgen {
  namespace io {
    ...
    TextHandler::TextHandler(const ParametersList& params)
      ...
      for (const auto& key : hist_vars.keys()) {
        ...
        if (vars.size() == 1) {  // 1D histogram
          ...
          CG_INFO("TextHandler") << "Booking a 1D histogram with " << utils::s("bin", nbins_x) << " between " << min_x
                                 << " and " << max_x << " for \"" << key << "\".";
          ...
          CG_INFO("TextHandler") << "Booking a 2D correlation plot with " << utils::s("bin", nbins_x + nbins_y)
    ...
    void TextHandler::operator<<(const Event& ev) {
      ...
      for (const auto& h_var : hists_)
        gsl_histogram_increment(h_var.second.get(), browser_.get(ev, h_var.first.name[0]));
      for (const auto& h_var : hists2d_)
        gsl_histogram2d_increment(
            h_var.second.get(), browser_.get(ev, h_var.first.name[0]), browser_.get(ev, h_var.first.name[1]));
      ++num_evts_;
    }

    std::string TextHandler::textHistogram(const hist1d_t& hist_info) {
      ...
      os << "plot of \"" << var << "\"\n"
         ...
         << std::setw(PLOT_WIDTH - 11) << std::left << (hist_info.first.log ? "logarithmic scale" : "linear scale")
      ...
      for (size_t i = 0; i < nbins; ++i) {
        ...
        os << "\n"
           << utils::format("[%7.2f,%7.2f):", min, max) << std::string(val, PLOT_CHAR)
           << std::string(PLOT_WIDTH - val, ' ') << ": " << utils::format("%6.2e", value) << " +/- "
    ...
    std::string TextHandler::textHistogram(const hist2d_t& hist_info) {
      auto hist = hist_info.second.get();
      std::ostringstream os;
      ...
      os << "d^2(sig)/d" << vars.at(0) << "/d" << vars.at(1) << " (pb/bin)\n"
         << sep << utils::format("%-5.2f", gsl_histogram2d_ymin(hist)) << std::string(nbins_y - 11, ' ')
      ...
      for (size_t i = 0; i < nbins_x; ++i) {
        ...
        for (size_t j = 0; j < nbins_y; ++j) {
          ...
          const double value_norm =
              hist_info.first.log ? (value == 0. ? 0. : log(value) / log(max_bin)) : value / max_bin;
