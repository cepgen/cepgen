...
#include "CepGenAddOns/EventInterfaces/PGSInterface.h"

namespace cepgen {
  namespace io {
    /**
     * \brief Export handler for PGS
     * \author Laurent Forthomme <laurent.forthomme@cern.ch>
     * \date Aug 2019
     */
    class PGSHandler : public ExportModule {
    public:
      explicit PGSHandler(const ParametersList&);
      ~PGSHandler();
    ...
    PGSHandler::PGSHandler(const ParametersList& params)
        : ExportModule(params),
      ...
      pgsevt_.numarg = 0;
      std::string("events").copy(pgsevt_.evtlum, sizeof(pgsevt_.evtlum));
      std::string("USER").copy(pgsevt_.optpgs, sizeof(pgsevt_.optpgs));
      //--- calculate mask for printout operation
      if (do_trig_)
    ...
    PGSHandler::~PGSHandler() { pgs_write_event_("end", 3); }

    void PGSHandler::initialise(const Parameters& params) {
      ...
      CG_INFO("PGSHandler") << "PGS initialised with input parameters card at:\n\n\t"
                            << "  \"" << pgsevt_.pgs_param_file << "\".\n\n\t"
                            << "Trigger emulation: " << do_trig_ << "\n\t"
                            << "Detector reconstruction: " << do_reco_ << ".";
    }

    void PGSHandler::operator<<(const Event& ev) {
      ...
      if (do_reco_) {
        //--- perform reconstruction; results stored in common block
        pgs_recon_();
      }
...
REGISTER_IO_MODULE("pgs", PGSHandler)
