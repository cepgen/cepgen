  ...
  namespace ctml {
    ...
    public:
      ...
      inline static ParametersDescription description() {
        ...
        desc.add<bool>("bare", false)
            .setDescription("generate a bare version (without <html>/<head>/<body> attributes)");
        return desc;
      }
      ...
      inline std::string describe() override {
        ...
        for (const auto& cat : categories_) {
          container_.AppendChild(
              CTML::Node("a").SetAttribute("name", cat.first).AppendChild(CTML::Node("h2", cat.second.title)));
          CTML::Node mods("p");
            ...
            mods.AppendChild(
                CTML::Node("a")
                    ...
                    .AppendChild(CTML::Node("span").AppendChild(moduleDescription(
                        mod.second,
                        cat.second.modules_indices.count(mod.first) > 0 ? cat.second.modules_indices.at(mod.first)
                                                                        : -1))));
