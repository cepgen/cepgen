...
std::set<double> Histogram::extractBins(BinMode mode,
  ...
  for (size_t i = 0; i < num_bins; ++i) {
    ...
    if (mode == low || mode == both)
      bins.insert(range.min());
    if (mode == high || mode == both)
      bins.insert(range.max());
  }
  const auto exp_bins = mode == both ? num_bins + 1 : num_bins;
  if (bins.size() != exp_bins)
    CG_WARNING("Histogram:extractBins") << "Invalid number of values to bin ranges: got " << bins.size()
                                        << ", expecting " << exp_bins << ".";
  return bins;
}
