...
void GridDrawer::draw(const GridHandler<3, N>& grid, const Drawer::Mode& mode) {
  ...
  for (size_t i = 0; i < N; ++i) {
    ...
    plots[i][0].zAxis().setLabel(format("var%d", i));
    plots[i][1].xAxis().setLabel("x0");
    plots[i][1].yAxis().setLabel("x2");
    plots[i][1].zAxis().setLabel(format("var%d", i));
    plots[i][2].xAxis().setLabel("x1");
    plots[i][2].yAxis().setLabel("x2");
    plots[i][2].zAxis().setLabel(format("var%d", i));
  }
...
template void GridDrawer::draw(const GridHandler<1>&, const Drawer::Mode&);
template void GridDrawer::draw(const GridHandler<3>&, const Drawer::Mode&);
