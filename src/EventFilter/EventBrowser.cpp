...
double EventBrowser::variable(const Event&, const Particle& part1, const Particle& part2, const std::string& var) const {
...
double EventBrowser::variable(const Event& ev, const std::string& var) {
  ...
  if (var == "nob1" || var == "nob2") {
    const auto& beam_particles = ev(var == "nob1" ? Particle::Role::OutgoingBeam1 : Particle::Role::OutgoingBeam2);
    return static_cast<double>(std::count_if(beam_particles.begin(), beam_particles.end(), [](const auto& part) {
      return static_cast<int>(part.status()) > 0;
    }));
  }
  if (var == "met")
