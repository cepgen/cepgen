...
namespace cepgen {
  ...
  std::complex<double> NachtmannAmplitudes::amplitudeSM(const Helicities& hel) const {
    if (hel.lam3 == 0 && hel.lam4 == 0)  // longitudinal-longitudinal
      return 1i * kin_.invA * kin_.inv_gamma2 *
             ((kin_.gamma2 + 1.) * (1. - hel.lam1 * hel.lam2) * kin_.sin_theta2 - (1. + hel.lam1 * hel.lam2));

    if (hel.lam4 == 0)  // transverse-longitudinal
      return -1i * kin_.invA * M_SQRT2 * kin_.inv_gamma * double(hel.lam1 - hel.lam2) *
             (1. + hel.lam1 * hel.lam3 * kin_.cos_theta) * kin_.sin_theta;

    if (hel.lam3 == 0)  // longitudinal-transverse
      return -1i * kin_.invA * M_SQRT2 * kin_.inv_gamma * double(hel.lam2 - hel.lam1) *
             (1. + hel.lam2 * hel.lam4 * kin_.cos_theta) * kin_.sin_theta;

    // transverse-transverse
    return -0.5i * kin_.invA *
           (2. * kin_.beta * float(hel.lam1 + hel.lam2) * (hel.lam3 + hel.lam4) -
  ...
  std::complex<double> NachtmannAmplitudes::amplitudeW(const Helicities& hel) const {
    if (hel.lam3 == 0 && hel.lam4 == 0)  // longitudinal-longitudinal
      return 3.i * kin_.shat * eft_ext_.s1 * M_SQRT2 * constants::G_F * kin_.invA * kin_.inv_gamma2 * kin_.sin_theta2 *
             (1. + hel.lam1 * hel.lam2);

    if (hel.lam4 == 0)  // transverse-longitudinal
      return 1.5i * kin_.shat * eft_ext_.s1 * constants::G_F * kin_.invA * kin_.inv_gamma * kin_.sin_theta *
    ...
    // transverse-transverse
    return 0.75i * kin_.shat * eft_ext_.s1 * M_SQRT2 * constants::G_F *
  ...
  std::complex<double> NachtmannAmplitudes::amplitudeWbar(const Helicities& hel) const {
    if (hel.lam3 == 0 && hel.lam4 == 0)  // longitudinal-longitudinal
    ...
    if (hel.lam4 == 0)  // transverse-longitudinal
      return 1.5 * kin_.shat * eft_ext_.s1 * constants::G_F * kin_.inv_gamma * kin_.invA * kin_.sin_theta *
             (kin_.beta * float(hel.lam1 - hel.lam2) * hel.lam3 +
              kin_.cos_theta * (2 * kin_.beta + (2. - kin_.beta2) * float(hel.lam1 + hel.lam2) * hel.lam3));

    if (hel.lam3 == 0)  // longitudinal-transverse
      return 1.5 * kin_.shat * eft_ext_.s1 * constants::G_F * kin_.inv_gamma * kin_.invA * kin_.sin_theta *
             (kin_.beta * float(hel.lam2 - hel.lam1) * hel.lam4 +
              kin_.cos_theta * (2 * kin_.beta + (2. - kin_.beta2) * float(hel.lam2 + hel.lam1) * hel.lam4));
    ...
    return -1.5 * kin_.shat * eft_ext_.s1 * M_SQRT2 * constants::G_F * kin_.invA *
           (2 * kin_.sin_theta2 * (hel.lam1 + hel.lam2 - kin_.beta * (hel.lam3 + hel.lam4)) +
            kin_.inv_gamma2 * (float(hel.lam1 + hel.lam2) * (kin_.cos_theta2 * (2 + hel.lam3 * hel.lam4) - 1) -
                               kin_.beta * (kin_.cos_theta2 + hel.lam1 * hel.lam2) * (hel.lam3 + hel.lam4)));
  }

  std::complex<double> NachtmannAmplitudes::amplitudephiW(const Helicities& hel) const {
    const double invB = 1. / (kin_.shat - eft_ext_.mH * eft_ext_.mH);
    if (hel.lam3 == 0 && hel.lam4 == 0)  // longitudinal-longitudinal
      return -0.25i * kin_.shat * kin_.shat * eft_ext_.s1 * eft_ext_.s1 * M_SQRT2 * constants::G_F * invB *
    ...
    // transverse-transverse
    return -0.125i * kin_.shat * kin_.shat * eft_ext_.s1 * eft_ext_.s1 * M_SQRT2 * constants::G_F * kin_.inv_gamma2 *
           invB * (1. + hel.lam1 * hel.lam2) * (1. + hel.lam3 * hel.lam4);
  }

  std::complex<double> NachtmannAmplitudes::amplitudeWB(const Helicities& hel) const {
    const double invB = 1. / (kin_.shat - eft_ext_.mH * eft_ext_.mH);
    if (hel.lam3 == 0 && hel.lam4 == 0)  // longitudinal-longitudinal
      return 2.i * kin_.invA * eft_ext_.c1() / eft_ext_.s1 *
    ...
    if (hel.lam4 == 0)  // transverse-longitudinal
      return 0.5i * kin_.gamma * M_SQRT2 * invB * eft_ext_.c1() / eft_ext_.s1 * kin_.sin_theta *
             ((hel.lam2 - hel.lam1) * (1. + kin_.inv_gamma2) +
              (kin_.beta * float(hel.lam1 + hel.lam2) + 2 * hel.lam3 * (hel.lam1 * hel.lam2 - kin_.inv_gamma2)) *
                  kin_.cos_theta);

    if (hel.lam3 == 0)  // longitudinal-transverse
      return 0.5i * kin_.gamma * M_SQRT2 * invB * eft_ext_.c1() / eft_ext_.s1 * kin_.sin_theta *
             ((hel.lam1 - hel.lam2) * (1. + kin_.inv_gamma2) +
              (kin_.beta * float(hel.lam2 + hel.lam1) + 2 * hel.lam4 * (hel.lam2 * hel.lam1 - kin_.inv_gamma2)) *
                  kin_.cos_theta);

    // transverse-transverse
    return -0.5i * kin_.invA * eft_ext_.c1() / eft_ext_.s1 *
               (kin_.beta * float(hel.lam1 + hel.lam2) * (hel.lam3 + hel.lam4) * (1. + kin_.cos_theta2) +
           ...
           0.25i * kin_.shat * kin_.shat * M_SQRT2 * constants::G_F * kin_.inv_gamma2 * invB * eft_ext_.s1 *
               eft_ext_.c1() * (1. + hel.lam1 * hel.lam2) * (1. + hel.lam3 * hel.lam4);
  }

  std::complex<double> NachtmannAmplitudes::amplitudeWbarB(const Helicities& hel) const {
    CG_WARNING("NachtmannAmplitudes") << "Mode " << mode_ << " is not yet properly handled!";
    const double invB = 1. / (kin_.shat - eft_ext_.mH * eft_ext_.mH);
    if (hel.lam3 == 0 && hel.lam4 == 0)  // longitudinal-longitudinal
      return 2 * eft_ext_.c1() / eft_ext_.s1 * kin_.gamma2 * float(hel.lam1 + hel.lam2) -
             0.5 * kin_.shat * kin_.shat * M_SQRT2 * constants::G_F /* /e^2 */ * eft_ext_.s1 * eft_ext_.c1() *
                 (1. + kin_.beta2) * float(hel.lam1 + hel.lam2);

    if (hel.lam4 == 0)  // transverse-longitudinal
      return 0.5 * kin_.invA * kin_.gamma * M_SQRT2 * eft_ext_.c1() / eft_ext_.s1 * kin_.sin_theta *
             (kin_.beta * (hel.lam2 - hel.lam1) * hel.lam3 -
              kin_.cos_theta * (2. * kin_.beta + kin_.beta2 * float(hel.lam1 + hel.lam2) * hel.lam3));
    ...
    return kin_.invA * eft_ext_.c1() * eft_ext_.c1() / eft_ext_.s1 *
               (hel.lam3 * float(hel.lam1 + hel.lam2) + kin_.beta * (hel.lam1 * hel.lam2 + kin_.cos_theta2)) *
               (hel.lam3 + hel.lam4) -
