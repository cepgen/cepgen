...
namespace cepgen {
  ...
  std::complex<double> NachtmannAmplitudes::amplitudeSM(const Kinematics& kin, const Helicities& hel) const {
    ...
    if (hel.lam4 == 0)  // transverse-longitudinal
      return -std::complex<double>(0., 1.) * G_EM_SQ * M_SQRT2 * kin.invA * kin.inv_gamma *
             double(hel.lam1 - hel.lam2) * (1. + hel.lam1 * hel.lam3 * kin.cos_theta) * kin.sin_theta;
