  ...
  namespace Hadroniser
  {
    ...
    class Jetset7Hadroniser : public GenericHadroniser
    {
      ...
      private:
         ...
         * \param[in] pdgid PDG id of the parton/particle
         */
        inline static double ulmass( int pdgid ) { return static_cast<double>( ulmass_( pdgid ) ); }
        /**
         * Administrate the fragmentation and decay chain.
         *  It may be called several times, but only entries which have not yet been treated (more precisely, have 1 \f$\leq\f$ KS \f$\leq\f$ 10) can be affected by further calls.
         *  This may apply if more jets/particles have been added by the user, or if particles previously considered stable are now allowed to decay.
         *  The actions that will be taken during a LUEXEC call can be tailored extensively via the LUDAT1 - LUDAT3 commonblocks, in particular by setting the MSTJ values suitably.
         */
        inline static void luexec() { luexec_(); }
        /**
         * Set the value of any variable residing in the commmonblocks LUJETS, LUDAT1, LUDAT2, LUDAT3, LUDAT4, or LUDATR.
         *  This is done in a more controlled fashion than by directly including the commonblocks in the user program, in that array bounds are checked and the old and new values for the variable changed is written to the output for reference.
         * \param[in] line The line to be parsed and fed to the Jetset instance
         */
        inline static void lugive( const std::string& line ) { lugive_( line.c_str(), line.length() ); }
        /**
         * Give the charge for a parton/particle
         * \param[in] pdgid PDG id of the parton/particle
         */
        inline static float luchge( int pdgid ) { return luchge_( pdgid )/3.; }
        /**
         * The \a mlist parameter can take these values:
         *  - 0: writes a header with program version number and last date of change; is mostly for internal use.
         *  - 1: gives a simple list of current event record, in an 80 column format suitable for viewing directly on the computer terminal.
         ...
         *    For an entry which has decayed/fragmented (KS = 11 - 20), this particle name is given within parantheses.
         *     Similarly, a documentation line (KS = 21 - 30) has the name enclosed in expression signs (!...!) and an event/jet axis information line the name within inequality signs (<...>).
         *     If the last character of the name is a ?, it is a signal that the complete name has been truncated to fit in, and can therefore not be trusted; this is very rare.
         ...
         * \param[in] mlist Determines what is to be listed (see detailed description)
         */
        inline static void lulist( int mlist ) { lulist_( mlist ); }
        /**
         * Give the parton/particle name (as a character string).
         * \param[in] pdgid PDG id of the parton/particle
         */
        inline static std::string luname( int pdgid ) {
          char out[NAME_CHR];
          std::string s;
          luname_( pdgid, out, NAME_CHR );
         ...
         * \param[in] njoin Number of particles to be joined by one string
         * \param[in] ijoin List of particles to join in the colour flow.
         *  An one-dimensional array, of size at least \a njoin.
         *  The \a njoin first numbers are the positions of the partons that are to be joined, given in the order the partons are assumed to appear along the string.
         *  If the system consists entirely of gluons, the string is closed by connecting back the last to the first entry.
         * \note Only one string (i.e. one colour singlet) may be defined per call, but one is at liberty to use any number of \a lujoin calls for a given event.
         *  The program will check that the parton configuration specified makes sense, and not take any action unless it is.
         *  Note, however, that an initially sensible parton configuration may become nonsensical, if only some of the partons are reconnected, while the others are left unchanged.
         */
        inline static void lujoin( int njoin, int ijoin[2] ) { return lujoin_( njoin, *ijoin ); }
