...
namespace CepGen
{
  namespace OutputHandler
  {
    ...
    LHEFHandler::LHAevent::feedEvent( unsigned short proc_id, const Event& ev, bool full_event )
    {
      ...
      const Particle& ip1 = ev.getOneByRole( Particle::IncomingBeam1 ), &op1 = ev.getOneByRole( Particle::OutgoingBeam1 );
      const Particle& ip2 = ev.getOneByRole( Particle::IncomingBeam2 ), &op2 = ev.getOneByRole( Particle::OutgoingBeam2 );
      const double q2_1 = -( ip1.momentum()-op1.momentum() ).mass2(), q2_2 = -( ip2.momentum()-op2.momentum() ).mass2();
