...
namespace cepgen
{
  ...
  std::string
  LoggedException::message() const
  {
    return message_.str();
  }

  std::string
  LoggedException::from() const
  {
    return from_;
  }

  int
  LoggedException::errorNumber() const
  {
    return error_num_;
  }

  Exception::Type
  LoggedException::type() const
  {
    return type_;
  }

  std::ostream&
  LoggedException::dump( std::ostream& os ) const
  {
