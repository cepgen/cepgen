...
namespace CepGen
{
  namespace Cards
  {
    ...
    PythonHandler::parseIncomingKinematics( PyObject* kin )
    {
      ...
      if ( hi_beam1.size() == 2 )
        params_.kinematics.incoming_beams.first.hi = HeavyIon{ (unsigned short)hi_beam1[0], (unsigned short)hi_beam1[1] };
      fillParameter( kin, "heavyIonB", hi_beam2 );
      if ( hi_beam2.size() == 2 )
        params_.kinematics.incoming_beams.second.hi = HeavyIon{ (unsigned short)hi_beam2[0], (unsigned short)hi_beam2[1] };
    }
